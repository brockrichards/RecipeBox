...
cp .\temp\coeus\RecipeBox-api\clean.ps1
cp .\temp\coeus\RecipeBox-api\format.ps1
cp .\temp\coeus\RecipeBox-api\create-release.ps1
cp .\temp\coeus\RecipeBox-api\generate-changelog.ps1
cp .\temp\coeus\RecipeBox-api\update-nugetpackages.ps1
cp .\temp\coeus\RecipeBox-api\src\.editorconfig .\src\.editorconfig
cp .\temp\coeus\RecipeBox-api\.gitignore

Remove-IfItemExists update-template.ps1

#cp .\temp\coeus\RecipeBox-api\.gitattributes
#git commit -m "Saving files before refreshing line endings" .gitattributes
#git add --renormalize .

cp .\temp\coeus\RecipeBox-api\build.ps1
cp .\temp\coeus\RecipeBox-api\dependabot.ps1
cp .\temp\coeus\RecipeBox-api\update-targetframework.ps1

#((Get-Content -path build.ps1 -Raw) -replace 'Acme.RecipeBox',$repository) | Set-Content -NoNewline -Path build.ps1 -Encoding utf8

if ($hasDatabase) {
	cp .\temp\coeus\RecipeBox-api\add-migration.ps1
	cp .\temp\coeus\RecipeBox-api\generate-sql.ps1
	cp .\temp\coeus\RecipeBox-api\generate-sqltriggers.ps1
	#git mv .\Generate-SqlTriggers.ps1 .\generate-sqltriggers.ps1
	cp .\temp\coeus\RecipeBox-api\remove-migration.ps1
	cp .\temp\coeus\RecipeBox-api\repository.psm1
	cp .\temp\coeus\RecipeBox-api\update-database.ps1

#	((Get-Content -path generate-sqltriggers.ps1 -Raw) -replace 'Acme.RecipeBox',$repository) | Set-Content -NoNewline -Path generate-sqltriggers.ps1 -Encoding utf8
...
if (Test-Path -path ".\\src\\sql\\table\\000-AuditLogTransaction.table.sql") {
	cp .\temp\coeus\RecipeBox-api\src\sql\table\000-AuditLogTransaction.table.sql src\sql\table
	cp .\temp\coeus\RecipeBox-api\src\sql\table\001-AuditLog.table.sql src\sql\table
}
