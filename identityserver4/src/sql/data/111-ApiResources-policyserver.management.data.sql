...
if (not exists(select * from auth.[ApiResources] where name=@name))
  ...
  -- "[Value]" contains a base64 encoding of a SHA256 hash of a random string.
	...
	-- policypass
	INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, N'policyserver.management', CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'fpfIn4boENE27x2z75Ho6wGFAJ9ZSCt1oCf4/FY6eLk=')
...
if (not exists(select * from auth.[ApiResources] where name=@name))
  ...
  -- "[Value]" contains a base64 encoding of a SHA256 hash of a random string.
	-- Generate the [Value] using that password in https://quickhash.com/ (use SHA-256 hashing, with the output of base64 encoding)
	-- policypass
	INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, N'policyserver.externalsearch', CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'fpfIn4boENE27x2z75Ho6wGFAJ9ZSCt1oCf4/FY6eLk=')
