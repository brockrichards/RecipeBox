...
if (not exists(select * from auth.[ApiResources] where name=@name))
  BEGIN
	...
	-- secret
	INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'FWBbELvuVqUgxiv+nUTFNrnFhud5ajnPmicCqISTmBc=')
  END
...
if (exists(select * from [AUTH].[ApiResources] where Name=@name))
  BEGIN
	...
	if (not exists(SELECT * FROM [AUTH].[ApiResourceSecrets] where ApiResourceId=@id AND Type = 'SharedSecret'))
		BEGIN
			-- secret
			INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'SharedSecret', N'FWBbELvuVqUgxiv+nUTFNrnFhud5ajnPmicCqISTmBc=')
		END
  END
