...
--select @hb as [BinaryHash], @h64 as [64Hash];
	...
	INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'fdo4ngK/EdMopuLVn6E9IsowjAnQsGMGPxjE8idKMGw=')
  END
...
if not exists (select top 1 1 from Auth.ApiResourceSecrets where ApiResourceId = @id and [Description] = 'identity introspection') begin
	insert into Auth.ApiResourceSecrets (ApiResourceId, [Description], Expiration, [Type], [Value]) values (@id, 'identity introspection', '2099-12-31', 'SharedSecret', 'FWBbELvuVqUgxiv+nUTFNrnFhud5ajnPmicCqISTmBc=')
end