...
export class RecipeBoxClient {
    ...
    getAuthorization(): Observable<AuthorizationResponse> {
        return this.http.get<AuthorizationResponse>(`${this.config.RecipeBoxApi?.url}/api/v1/authorization`, {
            context: new HttpContext().set(AUTHENTICATED_REQUEST, true),
        });
    }

    createOrders(order: OrderRequest): Observable<OrderReponse> {
        return this.http.post<OrderReponse>(`${this.config.RecipeBoxApi?.url}/api/v1/orders`, order, { context: new HttpContext().set(AUTHENTICATED_REQUEST, true) });
    }

    getOrders(): Observable<PagedResponse<OrderReponse>> {
        return this.http.get<PagedResponse<OrderReponse>>(`${this.config.RecipeBoxApi?.url}/api/v1/orders`, {context: new HttpContext().set(AUTHENTICATED_REQUEST, true)});
    }
}
